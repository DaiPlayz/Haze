...
function guiLibrary.saveCFG(Name: string)
	...
	writefile('Haze/configs/moonconfigs/'..game.PlaceId..'.json', httpService:JSONEncode(guiLibrary.Config))
end

function guiLibrary.loadCFG(Name: string)
	if runService:IsStudio() then return end

	if isfile('Haze/configs/moonconfigs/'..game.PlaceId..'.json') then
		guiLibrary.Config = httpService:JSONDecode(readfile('Haze/configs/moonconfigs/'..game.PlaceId..'.json'))
	end
end
